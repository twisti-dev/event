...
class GlobalCobbleCountBossbar(
    ...
    private val bossBar = Bukkit.createBossBar("Progress", BarColor.GREEN, BarStyle.SOLID).apply {
        this.progress = (MiningPlayerManager.getGlobalCobbleMined().toDouble() / 500_000).coerceAtMost(1.0)
        this.isVisible = true
    }

    private val timer = Bukkit.getScheduler().runTaskTimer(plugin, Runnable {
        bossBar.progress = (MiningPlayerManager.getGlobalCobbleMined().toDouble() / 500_000).coerceAtMost(1.0)
